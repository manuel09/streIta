...
class Loonex : MainAPI() {
    ...
    override suspend fun getMainPage(page: Int, request: MainPageRequest): HomePageResponse {
        ...
        if (lists.isEmpty()) {
            val allCards = doc.select("a[href*=\"?cartone=\"]").mapNotNull { parseCard(it) }
    ...
    override suspend fun search(query: String): List<SearchResponse> {
        ...
        val results = doc.select("a[href*=\"?cartone=\"]").mapNotNull { parseCard(it) }
        return results
    }
    ...
    override suspend fun load(url: String): LoadResponse {
        ...
        if (isMovie) {
            ...
            val episodeUrl = "$guardaBase/?id=$episodeId"
        ...
        if (tabPanes.isNotEmpty()) {
            ...
            for (link in episodeLinks) {
                ...
                if (epRow != null) {
                    ...
                    val epUrl = "$guardaBase/?id=$episodeId"
